...
export class SyncService {
  ...
  private async findExistingSyncRecord(params: {
    collection: ReturnType<Connection['collection']>;
    operation: SyncOperationDto;
    payload: Record<string, unknown>;
    businessId: unknown;
    ownerId: string;
    serverObjectId: Types.ObjectId | null;
  }) {
    const {
      collection,
      operation,
    ...
    const orFilters: Record<string, unknown>[] = [
      ...(serverObjectId
        ? [
            {
              _id: serverObjectId,
              ...this.uploadOwnershipFilter(entity, ownerId),
            },
          ]
        : []),
      ...
      ...(entity === 'routeSessions'
        ? [{ routeSessionId: businessId, userId: ownerId }]
        : []),
    ];

    /**
     * sale_items has unique index by saleId + productId.
     * If an old/offline duplicate exists, sync must merge into the
     * existing row instead of trying another insert.
     */
    if (entity === 'orderItems') {
      ...
      if (saleId && productId) {
        orFilters.push({ saleId, productId });
      }
    }

    /**
     * Inventory transactions must also be idempotent.
     */
    if (entity === 'inventoryTransactions') {
      const transactionId = String(payload.transactionId ?? '');

      if (transactionId) {
        orFilters.push({ transactionId });
      }
    }

    return collection.findOne({ $or: orFilters });
  }

  private buildMergedOrderItemPayload(
    existing: Record<string, unknown>,
    payload: Record<string, unknown>,
  ) {
    const { _id: _existingObjectId, ...safeExisting } = existing;

    const unitQtyInCase = Math.max(
      toFiniteNumber(
        payload.unitQtyInCase,
        toFiniteNumber(existing.unitQtyInCase, 1),
      ),
      1,
    );

    const caseQty =
      toFiniteNumber(existing.caseQty) + toFiniteNumber(payload.caseQty);

    const pieceQty =
      toFiniteNumber(existing.pieceQty) + toFiniteNumber(payload.pieceQty);

    const casePrice = toFiniteNumber(
      payload.casePrice,
      toFiniteNumber(existing.casePrice),
    );
    ...
    const pieceNetWeight = toFiniteNumber(
      payload.pieceNetWeight,
      toFiniteNumber(existing.pieceNetWeight),
    );
    ...
    for (const operation of operations) {
      ...
      try {
        ...
        if (!WRITABLE_ENTITIES.has(operation.entity))
          throw new Error(
            `Entity is not writable from offline sync: ${operation.entity}`,
          );
        ...
        if (operation.operation === 'CREATE') {
          if (existing) {
            ...
            const shouldMergeOfflineChanges =
              operation.entity === 'attendance' ||
              operation.entity === 'orderItems' ||
              wasCreatedByOfflineSync;
            const existingRecordChanges =
              operation.entity === 'orderItems'
                ? this.buildMergedOrderItemPayload(existing, payload)
                : shouldMergeOfflineChanges
                  ? payload
                  : {};
        ...
        if (conflictResult.hasConflict) {
          ...
          await this.markOfflineSyncAuditFailed(auditObjectId, result.error);
        ...
        /**
         * Final safety for already-existing duplicate local queue rows.
         * If MongoDB still throws duplicate key for sale_items, merge into the
         * existing sale item and mark the queue item as successfully synced.
         */
        if (operation.entity === 'orderItems' && mongoError?.code === 11000) {
          try {
            const collectionName =
              COLLECTIONS[operation.entity as keyof typeof COLLECTIONS];
            const collection = this.connection.collection(collectionName);
            ...
            if (saleId && productId) {
              const existingOrderItem = await collection.findOne({
                saleId,
                productId,
              });

              if (existingOrderItem) {
                const mergedPayload = this.buildMergedOrderItemPayload(
                  existingOrderItem,
                  payload,
                );

                const nextVersion = Number(existingOrderItem.version ?? 1) + 1;
                ...
                await collection.updateOne(
                  { _id: existingOrderItem._id },
                  {
                    $set: {
                      ...mergedPayload,
                      uuid: existingOrderItem.uuid ?? operation.localId,
                      isDeleted: false,
                      deletedAt: null,
                      lastSyncSource: 'OFFLINE',
                      lastSyncedAt: new Date(),
                ...
                const result = {
                  ...
                  localId: operation.localId,
                  success: true,
                  serverId: String(existingOrderItem._id),
                  version: nextVersion,
                ...
                await this.markOfflineSyncAuditSuccess(auditObjectId, {
                  serverId: String(existingOrderItem._id),
                  version: nextVersion,
                  after: { ...mergedPayload, version: nextVersion },
                  result,
                });
